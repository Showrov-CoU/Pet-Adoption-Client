...
import { FaEdit, FaPause, FaStop, FaTrash } from "react-icons/fa";
...
const AllDonations = () => {
  ...
  const { data: getallDonations = [], refetch } = useQuery({
    queryKey: ["getallDonations"],
    queryFn: async () => {
  ...
  const handlePause = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.patch(`/pause/${id}`).then((res) => {
          console.log(res.data);
          if (res.data.modifiedCount > 0) {
            refetch();
            Swal.fire({
    ...
    <div className="w-full mx-auto">
      <div className="w-full relative overflow-x-auto shadow-md sm:rounded-lg">
        <div className="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-white dark:bg-gray-900">
          <div>
            ...
            >
              ...
              Total Donation: {getallDonations?.length}
            </button>
          </div>
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {getallDonations?.map((myDonationCampData, idx) => (
              <tr
              ...
              >
                ...
                <td className="px-6 py-4 text-start">
                  <div className="flex justify-center gap-6 items-center">
                    ...
                    <Button
                      onClick={() => handlePause(myDonationCampData._id)}
                      color="success"
                    >
                      {/* {pause ? (
                        <FaStop size={16}></FaStop>
                      ) : (
                        <FaPause size={16}></FaPause>
                      )} */}
                      {myDonationCampData.pause ? (
                        <FaStop size={16}></FaStop>
                      ) : (
                        <FaPause size={16}></FaPause>
                      )}
